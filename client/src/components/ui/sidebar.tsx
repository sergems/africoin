...
function Sidebar({
  ...
  const swipeActive = React.useRef(false);
  ...
  const handleSwipeMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (deltaX < 0) {
      event.preventDefault();
      setSwipeOffset(Math.max(deltaX, -320));
    }
  };

  const handleSwipeEnd = (event: React.PointerEvent<HTMLDivElement>) => {
    if (!isMobile || !swipeStart.current) return;
    const shouldDismiss = swipeOffset <= -84;
    swipeStart.current = null;
    swipeActive.current = false;
    setSwipeOffset(0);
    ...
    if (shouldDismiss) setOpenMobile(false);
  };
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          data-slot="sidebar"
          data-mobile="true"
          className="bg-sidebar text-sidebar-foreground w-(--sidebar-width) touch-pan-y p-0 [&>button]:hidden"
          style={
            {
              "--sidebar-width": SIDEBAR_WIDTH_MOBILE,
              transform: swipeOffset ? `translateX(${swipeOffset}px)` : undefined,
              transitionDuration: swipeOffset ? "0ms" : undefined,
            } as React.CSSProperties
          }
          side={side}
        >
