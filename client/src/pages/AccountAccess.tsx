...
const inputClass = "mt-2 h-11 border-white/15 bg-white/[0.07] text-white placeholder:text-slate-500 focus:border-[#18b89a] focus:ring-[#18b89a]/20";
const selectClass = "mt-2 h-11 w-full rounded-lg border border-white/15 bg-[#0d2638] px-3 text-sm text-white outline-none transition focus:border-[#18b89a] focus:ring-2 focus:ring-[#18b89a]/20";
...
export default function AccountAccess({ mode }: { mode: AccessMode }) {
  ...
  const register = trpc.auth.register.useMutation({ onSuccess: () => { setActiveMode("login"); setPassword(""); toast.success("Compte créé. Vous pouvez maintenant vous connecter."); }, onError: error => toast.error(error.message) });
  const login = trpc.auth.login.useMutation({ onSuccess: async () => { await utils.auth.me.invalidate(); navigate("/dashboard"); toast.success("Bienvenue dans votre espace Africoin."); }, onError: error => toast.error(error.message) });
  ...
  return <div className="relative min-h-screen overflow-hidden bg-[#071a2a] px-5 py-8 text-white sm:px-8 lg:px-12"><div className="atg-grid pointer-events-none absolute inset-0 opacity-35" /><div className="pointer-events-none absolute -right-40 top-[-10rem] h-[34rem] w-[34rem] rounded-full bg-[#087f78]/20 blur-3xl" /><div className="relative mx-auto grid min-h-[calc(100vh-4rem)] max-w-[1440px] items-center gap-12 lg:grid-cols-[.8fr_1.2fr] lg:gap-20">
    <div className="hidden lg:block"><Link href="/" className="inline-flex items-center gap-2 text-sm font-semibold text-slate-300 transition hover:text-white"><ArrowLeft className="h-4 w-4" />Retour à l’accueil</Link><p className="atg-kicker mt-20 text-[#7ddcca]">AFRICOIN TRADING GROUP</p><h1 className="atg-page-title mt-5 max-w-xl font-extrabold">Un profil plus complet pour un parcours plus sûr.</h1><p className="mt-6 max-w-lg text-base leading-8 text-slate-300">Ces informations nous aident à préparer votre profil investisseur, votre adéquation et les contrôles KYC avant toute opération réelle.</p><div className="mt-12 grid max-w-md gap-6 border-t border-white/15 pt-6 text-sm text-slate-300"><Feature icon={<ShieldCheck />} text="Profil rattaché à la gouvernance et à la conformité." /><Feature icon={<LockKeyhole />} text="Mot de passe protégé par un hachage scrypt." /><Feature icon={<ArrowRight />} text="Marchés, watchlist et ordres en attente après inscription." /></div></div>
    <div className="mx-auto w-full max-w-2xl">{activeMode === "login" && <Link href="/" className="mb-5 inline-flex items-center gap-2 rounded-lg px-1 py-2 text-sm font-semibold text-slate-300 transition hover:text-white lg:hidden"><ArrowLeft className="h-4 w-4" />Retour à l’accueil</Link>}<Card className="w-full rounded-2xl border-white/15 bg-[#0d2638]/90 text-white shadow-[0_30px_100px_rgba(0,0,0,.28)] backdrop-blur-xl"><CardHeader className="border-b border-white/10 p-7 pb-6 sm:p-9 sm:pb-7"><div className="flex items-center gap-4"><div className="grid h-11 w-11 place-items-center bg-[#e6b93f] text-[#071a2a]"><UserRound className="h-5 w-5" /></div><div><p className="atg-kicker text-[#7ddcca]">Espace client</p><CardTitle className="mt-2 text-2xl font-extrabold tracking-[-.04em] text-white">{activeMode === "register" ? "Créer votre compte" : "Se connecter"}</CardTitle></div></div>{activeMode === "register" && <p className="mt-4 text-sm leading-6 text-slate-400">Informations personnelles et profil investisseur</p>}</CardHeader><CardContent className="space-y-5 p-7 sm:p-9">
      {activeMode === "register" && <>
        ...
        <label className="flex items-start gap-3 text-xs leading-5 text-slate-300"><input type="checkbox" checked={accepted} onChange={event => setAccepted(event.target.checked)} className="mt-1 accent-[#e6b93f]" />J’accepte les conditions d’utilisation, les informations sur les risques et le traitement de mes données pour l’ouverture du compte.</label>
      </>}
      ...
      <Button disabled={isPending || (activeMode === "register" && (!accepted || password.length < 8))} onClick={submit} className="h-12 w-full rounded-lg bg-[#e6b93f] font-extrabold text-[#071a2a] hover:bg-[#f1ca63]">{isPending ? "Traitement…" : activeMode === "register" ? "Créer mon compte" : "Accéder à mon espace"}<ArrowRight className="ml-2 h-4 w-4" /></Button>
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-white/10 pt-5 text-sm text-slate-400">{activeMode === "register" ? <button onClick={() => setActiveMode("login")} className="font-semibold transition hover:text-white">J’ai déjà un compte</button> : <button onClick={() => setActiveMode("register")} className="font-semibold transition hover:text-white">Créer un compte</button>}<Link href="/tarifs-risques-conditions" className="font-semibold transition hover:text-white">Tarifs & risques</Link></div>
      {activeMode === "login" && <p className="border-l-2 border-[#18b89a] bg-[#18b89a]/10 p-3 text-xs leading-5 text-slate-300">Connexion sécurisée par les identifiants Africoin. Les opérations réelles restent soumises à la conformité et aux validations applicables.</p>}
    </CardContent></Card></div>
  </div></div>;
}

function Field({ label, htmlFor, children }: { label: string; htmlFor: string; children: React.ReactNode }) { return <div><Label htmlFor={htmlFor} className="text-xs font-semibold text-slate-200">{label}</Label>{children}</div>; }
function Feature({ icon, text }: { icon: React.ReactNode; text: string }) { return <div className="flex items-center gap-3"><span className="text-[#e6b93f]">{icon}</span><span>{text}</span></div>; }
