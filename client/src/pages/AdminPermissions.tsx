...
export default function AdminPermissions() {
  ...
  return <div className="min-h-screen bg-[#f7f7f2] px-4 py-6 sm:px-8 lg:px-10"><div className="mx-auto max-w-[1500px]">
    <header className="flex flex-col gap-4 border-b border-slate-200 pb-6 lg:flex-row lg:items-end lg:justify-between">
      <div><Link href="/admin" className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-[#087f78] hover:underline"><ArrowLeft className="h-3.5 w-3.5" /> Administration</Link><p className="mt-4 text-xs font-semibold uppercase tracking-[0.22em] text-[#087f78]">Version 2.2 · gouvernance</p><h1 className="mt-3 text-3xl font-semibold tracking-tight text-[#0a2233]">Permissions & approbations</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Une matrice lisible et une règle des deux personnes pour les changements sensibles. Les demandes restent en attente tant que deux approbateurs distincts n’ont pas validé.</p></div>
      <div className="flex flex-col items-start gap-2"><Badge className="w-fit border border-[#b9ded4] bg-[#e9f6f2] px-4 py-2 text-[#087f78]"><ShieldCheck className="mr-2 h-4 w-4" />{roleLabel(user!.role)} · périmètre audité</Badge><p className="text-xs text-slate-500">{roleScope(user!.role)}</p></div>
    </header>
    ...
    <section className="mt-7 grid gap-6 xl:grid-cols-[1.1fr_.9fr]">
      <Card className="rounded-2xl border-slate-200 bg-white"><CardHeader><CardTitle className="text-xl text-[#0a2233]">Matrice de permissions</CardTitle><p className="text-sm text-slate-500">Les gardes sont appliquées côté serveur, avant chaque lecture ou mutation sensible.</p></CardHeader><CardContent><div className="overflow-x-auto"><table className="w-full min-w-[640px] text-left text-sm"><thead><tr className="border-b border-slate-100 text-xs uppercase tracking-[0.12em] text-slate-400"><th className="px-3 py-3 font-medium">Capacité</th>{roleOrder.map(role => <th key={role} className="px-3 py-3 font-medium">{roleLabel(role)}</th>)}</tr></thead><tbody>{Object.keys(permissionLabels).map(permission => <tr key={permission} className="border-b border-slate-50 last:border-0"><td className="px-3 py-3 font-medium text-[#0a2233]">{permissionLabels[permission as Permission]}</td>{roleOrder.map(role => <td key={role} className="px-3 py-3">{hasPermission(role, permission as Permission) ? <Badge className="bg-[#e9f6f2] text-[#087f78]">Autorisé</Badge> : <Badge variant="outline" className="text-slate-400">Refusé</Badge>}</td>)}</tr>)}</tbody></table></div></CardContent></Card>
      <Card className="rounded-2xl border-0 bg-[#0a2233] text-white shadow-xl"><CardHeader><CardTitle className="flex items-center gap-2 text-xl text-white"><LockKeyhole className="h-5 w-5 text-[#e6b93f]" />Règle des deux personnes</CardTitle></CardHeader><CardContent className="space-y-4 text-sm leading-6 text-slate-300"><p>Une demande sensible est créée par un opérateur autorisé, mais ne s’exécute jamais sur sa seule action.</p><div className="grid gap-3 sm:grid-cols-3 xl:grid-cols-1"><Rule step="01" text="Demandeur" detail="Justification obligatoire" /><Rule step="02" text="Premier avis" detail="Approver distinct" /><Rule step="03" text="Exécution" detail="Après le second avis" /></div><div className="rounded-2xl border border-[#e6b93f]/30 bg-[#e6b93f]/10 p-4 text-xs text-[#f7e0a4]">Auto-approbation, double décision du même compte et auto-blocage sont refusés par l’API.</div></CardContent></Card>
    </section>

    <section className="mt-7"><div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#087f78]">File de contrôle</p><h2 className="mt-1 text-2xl font-semibold text-[#0a2233]">Demandes en attente</h2></div><p className="text-xs text-slate-500">Actualisation automatique toutes les 10 secondes</p></div>{pending.length ? <div className="grid gap-4 lg:grid-cols-2">{pending.map(request => <ApprovalCard key={request.id} request={request} currentUserId={user!.id} currentRole={user!.role} note={note} setNote={setNote} isPending={decide.isPending} onDecide={decideRequest} />)}</div> : <div className="rounded-2xl border border-[#b9ded4] bg-[#e9f6f2] p-6 text-sm text-[#087f78]"><CheckCircle2 className="mb-2 h-5 w-5" />Aucune demande en attente. La file de gouvernance est à jour.</div>}</section>

    <section className="mt-7"><Card className="rounded-2xl border-slate-200 bg-white"><CardHeader><CardTitle className="text-xl text-[#0a2233]">Historique des décisions</CardTitle></CardHeader><CardContent>{history.length ? <div className="space-y-3">{history.map(request => <div key={request.id} className="flex flex-col gap-3 rounded-2xl border border-slate-100 bg-[#fbfcfb] p-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="font-semibold text-[#0a2233]">#{request.id} · {request.actionLabel}</span><Badge className={request.status === "approved" ? "bg-[#e9f6f2] text-[#087f78]" : "bg-rose-100 text-rose-700"}>{request.status === "approved" ? "Exécutée" : request.status === "rejected" ? "Rejetée" : request.status}</Badge></div><p className="mt-1 text-xs text-slate-500">Demandeur : {request.requester?.name || `#${request.requesterUserId}`} · {new Date(request.createdAt).toLocaleString("fr-FR")}</p><p className="mt-1 text-xs text-slate-500">{request.reason}</p></div><p className="shrink-0 text-xs text-slate-500">{request.decisions.length}/{request.requiredApprovals} avis</p></div>)}</div> : <p className="text-sm text-slate-500">Aucun historique de gouvernance.</p>}</CardContent></Card></section>
  </div></div>;
}
